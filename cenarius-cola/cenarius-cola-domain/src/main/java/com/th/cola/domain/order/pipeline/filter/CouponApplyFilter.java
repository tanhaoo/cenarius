...
public class CouponApplyFilter extends AbstractBizFilter<SalesOrderContext> {
    private final ISalesOrderPipelineGateway pipelineGateway;

    @Override
    protected void handle(SalesOrderContext context) {
        if (!context.getSalesOrder().getIsUseCoupon()) {
            return;
        }

        pipelineGateway.getPromotionGateway().useCoupon();
